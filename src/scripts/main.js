...
$(document).ready(function () {
  ...
  searchScript.src =
    "https://txpub.usgs.gov/DSS/search_api/1.1/api/search_api.min.js";
  ...
  function setSearchAPI() {
    search_api.on("load", function () {
      ...
      search_api.on("location-found", function (lastLocationFound) {
        ...
        var zoomlevel = 14;
        if (lastLocationFound.Category === "U.S. State or Territory")
          zoomlevel = 9;

        map.setView(
          [lastLocationFound.y + 0.01, lastLocationFound.x],
          zoomlevel
        );
        ...
        L.popup()
          ...
          .setContent(
            "<b>NAME:            </b> " +
              lastLocationFound.name +
              "<br/>" +
              "<b>CATEGORY:        </b> " +
              lastLocationFound.category +
              "<br/>" +
              "<b>STATE:           </b> " +
              lastLocationFound.state +
              "<br/>" +
              "<b>COUNTY:          </b> " +
              lastLocationFound.county +
              "<br/>" +
              "<b>LATITUDE:        </b> " +
              lastLocationFound.y +
              "<br/>" +
  ...
  var icon0 = L.icon({
    iconUrl: "images/markers/cont.png",
    iconAnchor: [7, 7],
    popupAnchor: [0, 0],
    iconSize: [16, 16],
  });
  ...
  var icon2 = L.icon({
    iconUrl: "images/markers/lime.png",
    iconAnchor: [10, 10],
    popupAnchor: [0, 2],
    iconSize: [20, 20],
  });
  var icon3 = L.icon({
    iconUrl: "images/markers/sky.png",
    iconAnchor: [10, 10],
    popupAnchor: [0, 2],
    iconSize: [20, 20],
  });
  ...
  // JSON DATA //
  $.getJSON("./data/LookupCount.json", function (json) {
  ...
  $.getJSON("./data/data.json", function (json) {
    ...
    for (var i = 0; i < json.items.length; i++) {
      ...
      // links to data //
      var link =
        "https://mn.water.usgs.gov/infodata/lowflow/disContData/" +
        a.site_no +
        ".txt";
      var link2 =
        "https://mn.water.usgs.gov/infodata/lowflow/contData/freqOutput/" +
        a.site_no +
        ".txt";
      var link3 =
        "https://mn.water.usgs.gov/infodata/lowflow/contData/logNormal/p" +
        a.site_no +
        ".pdf";
      var link4 =
        "https://mn.water.usgs.gov/infodata/lowflow/contData/logPearson/p" +
        a.site_no +
        ".pdf";
      var link5 =
        "https://mn.water.usgs.gov/infodata/lowflow/contData/stationDescription/" +
        a.site_no +
        ".txt";
      var linkNWIS =
        "https://waterdata.usgs.gov/monitoring-location/" + a.site_no + "/";

      var content =
        "<label>Gage #: </label><b>" +
        a.site_no +
        "<label>Station: </label><b>" +
        a.station_nm +
        "</b><div class='leaflet-popup-buttons'><a class='btn-sm btn-primary' target='_blank' href='" +
        link +
        "'> Data </a>" +
        "<a class='btn-sm btn-primary' target='_blank' href='" +
        linkNWIS +
        "'> NWIS </a></div>";
  ...
  Sites = L.esri.featureLayer({
    url: "https://pca-gis02.pca.state.mn.us/arcgis/rest/services/agol/ww_facility/MapServer/6",
    pointToLayer: function (feature, latlng) {
      //return L.marker(latlng, { icon:  iconFs} );
      var mypopup = L.popup().setContent(
        "Permit Number : " +
          feature.properties.permit_number +
          "<br>" +
          "Facility : " +
          feature.properties.ai_name +
          "<br>" +
          "Station # : " +
          feature.properties.si_designation +
          "<br>" +
          "<br>" +
          "Station description : " +
  ...
  Huc8 = L.esri.dynamicMapLayer({
    url: "https://gis1.wim.usgs.gov/server/rest/services/MNLowFlow/MNMNHuc8DarkRed/MapServer/",
    layers: [0],
  });
    ...
    if (map.hasLayer(Huc8)) {
      Huc8.identify()
        ...
        .run(function (error, featureCollection) {
          ...
          if (
            featureCollection.features[0].properties.STATES.indexOf("MN") >= 0
          ) {
            var infos = featureCollection.features[0].properties;
            //console.log(infos)
            var Huc8Popup = L.popup()
              .setLatLng(e.latlng)
              .setContent(
                "<p></p><p><b>HUC8: </b>" +
                  infos.HUC8 +
